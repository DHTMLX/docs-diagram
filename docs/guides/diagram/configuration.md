...
Check the full list of configuration properties of a [shape](shapes/configuration_properties.md) and [line](lines/configuration_properties.md).
...
:::note
Autoplacement will be applied only after the [`autoPlace()`](api/diagram/autoplace_method.md) method is applied. It works only in the default mode of the diagram and only for shapes. Autoplacement does not work if you use groups or swimlanes.
:::
