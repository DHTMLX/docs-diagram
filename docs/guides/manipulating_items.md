...
There is also the ability to configure settings for the auto-placement by using the [`autoplacement`](api/diagram/autoplacement_property.md) configuration option of Diagram and applying the `autoPlace()` method. The example below demonstrates how to generate and arrange a radial dataset using autoplacement.
...
You can check all available properties of shape objects in the [API section](shapes/configuration_properties.md).

**Related sample**: [Diagram. Data. Add/delete item](https://snippet.dhtmlx.com/8wi20uop)

## Getting an item

You can get the object of an item by passing its id to the [`getItem()`](api/data_collection/getitem_method.md)  method of the `data` object. For example:
...
You can get the index of an item by passing its id to the [`getIndex()`](api/data_collection/getindex_method.md) method:
...
If necessary, you can clear the diagram, by deleting all the items at once. For this, apply the [`removeAll()`](api/data_collection/removeall_method.md) method of the `data` object:
...
You can update the look and content of an item with the help of the [`update()`](api/data_collection/update_method.md) method of the `data` object:
...
You can check whether an item exists in the diagram via the [`exists()`](api/data_collection/exists_method.md) method of the `data` collection. The method takes the id of the item as a parameter and returns *true*, if the item exists:
...
You can get the list of ids of the currently selected items with the [`getIds()`](api/selection/getids_method.md) method of the `selection` object:
...
You can get the object of a selected item using the [`getItem()`](api/selection/getitem_method.md) method of the `selection` object. The method may take as an argument an object with the following parameter:
...
Whenever you need to clear the selection list without invoking events, use the [`clear()`](api/selection/clear_method.md) method:
...
There is a way to check the presence of an item in the list of selected Diagram items via API. Use the [`includes()`](api/selection/includes_method.md) method of the `selection` object for this purpose:
...
You can [read more about scrolling in DHTMLX Diagram](guides/diagram/scrolling_diagram.md).

## Finding the necessary item

You can use the API of the Diagram component to find the necessary item in the diagram. The [`find()`](api/data_collection/find_method.md) method of the `data` collection will help you to perform this task.
...
You can also find all the items that meet the set criteria via the [`findAll()`](api/data_collection/findall_method.md) of the `data` collection method. The method takes the same parameters as the `find()` method and returns an array of item objects:
...
To be able to manipulate the cells of a swimlane, you need to set the active swimlane. It can be done either via the [`setSwimlane()`](api/cell_manager/setswimlane_method.md) method of the cellManager object:

~~~jsx
diagram.cellManager.setSwimlane("main");
~~~

or via the [`add()`](api/selection/add_method.md) method of the selection object:

~~~jsx
diagram.selection.add("main");
~~~

After you finish operating the swimlane or its cells, you can apply the [`resetSwimlane()`](api/cell_manager/resetswimlane_method.md) method of the cellManager object to remove selection from the swimlane:
...
To remove a set of cells either as a row or column from the swimlane, use the [`remove()`](api/cell_manager/remove_method.md) method of the cellManager object. The method takes two parameters:
...
You can check whether such actions as adding, removing, or moving cells can be done over the specified cell via the [`validation()`](api/cell_manager/validation_method.md) method of the cellManager object:
...
You can get the id of the first cell of a column/row the cell belongs to. It can be done via the [`getCellId()`](api/cell_manager/getcellid_method.md) method of the cellManager object. The method takes two parameters:
...
You can also get the id of a cell the subheader belongs to by using the [`getSubHeaderCellId()`](api/cell_manager/getsubheadercellid_method.md) method of the cellManager object. The method takes the id of the subheader of a swimlane as a parameter:
...
When you need to get the index of a cell of a swimlane, use the [`getCellIndex()`](api/cell_manager/getcellindex_method.md) method of the cellManager object. The method takes two parameters:
...
You can also get the index of a cell the subheader belongs to by using the [`getSubHeaderCellIndex()`](api/cell_manager/getsubheadercellindex_method.md) method of the cellManager object. The method takes the id of the subheader of a swimlane as a parameter:
