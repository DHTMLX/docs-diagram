...
description: Informieren Sie sich in der Dokumentation der DHTMLX JavaScript Diagram-Bibliothek über das Steuerelement Pointer view der Editbar. Durchsuchen Sie Entwicklerhandbücher und die API-Referenz, testen Sie Codebeispiele und Live-Demos, und laden Sie eine kostenlose 30-Tage-Testversion der DHTMLX Suite herunter.
---
...
@short: Das komplexe Steuerelement zum Anzeigen und Verwalten der Verbindungspfeile
...
:::info
Das Steuerelement **Pointer view** ist nur für das Element `line` im *default*-Modus des Diagram-Editors verfügbar.
:::
...
- `type` - (erforderlich) der Typ eines Steuerelements. Setzen Sie ihn auf `"pointerView"`
...
- `$properties` - (optional) ermöglicht das Überschreiben von Werten der [Basissteuerelemente](api/diagram_editor/editbar/basic_controls_overview.md) innerhalb eines komplexen Steuerelements. Sie können die folgenden Elemente des Steuerelements **Pointer view** basierend auf den Basissteuerelementen konfigurieren:
    - `backArrow` - ([toggleGroup](api/diagram_editor/editbar/basic_controls/togglegroup.md)) legt den Pfeiltyp am Ende der Verbindungslinie fest
    - `forwardArrow` - ([toggleGroup](api/diagram_editor/editbar/basic_controls/togglegroup.md)) legt den Pfeiltyp am Anfang der Verbindungslinie fest
