...
MCP-сервер DHTMLX (Model Context Protocol) решает эту проблему, предоставляя ИИ-инструментам прямой доступ к актуальной документации Diagram. Работаете ли вы с [дорожками](/swimlanes/), [пользовательскими фигурами](/shapes/custom_shape), [Diagram Editor](/guides/diagram_editor/initialization) или любой другой частью библиотеки — ассистент получает актуальные справочные материалы перед формированием ответа.
