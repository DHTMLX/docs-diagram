...
- `callback` - (обязательный) функция обратного вызова, которая будет применена к каждому родительскому элементу указанного элемента
- `self` - (необязательный) определяет, должна ли функция обратного вызова также обрабатывать сам указанный элемент; по умолчанию — *false*
