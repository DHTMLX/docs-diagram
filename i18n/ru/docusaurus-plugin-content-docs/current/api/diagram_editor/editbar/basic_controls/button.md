...
- `$layout` - (необязательно) - функция обратного вызова, которая позволяет задавать структуру элемента управления. Возвращает конфигурацию элемента управления Form [Button](https://docs.dhtmlx.com/suite/form/button/). Вызывается со следующим параметром:
