...
title: Editor 的 beforeGroupMove 事件
description: 您可以在 DHTMLX JavaScript Diagram 库的文档中了解 editor 的 beforeGroupMove 事件。浏览开发者指南和 API 参考，试用代码示例和在线演示，并下载 DHTMLX Diagram 的免费 30 天评估版本。
---
...
该事件的回调函数使用一个包含以下参数的对象进行调用：
  ...
  - `coords` - 一个包含移动前组或泳道位置的 `x` 和 `y` 坐标的对象，其中：
    - `x` - 组/泳道的水平位置，从左向右移动
    - `y` - 组/泳道的垂直位置，从上向下移动
  - `event` - 事件对象
...
回调函数返回 `false` 以阻止组或泳道被移动；否则返回 `true`
