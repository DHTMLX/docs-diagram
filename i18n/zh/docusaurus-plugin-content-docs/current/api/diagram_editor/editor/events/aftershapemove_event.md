...
title: Editor 的 afterShapeMove 事件
description: 您可以在 DHTMLX JavaScript Diagram 库的文档中了解 editor 的 afterShapeMove 事件。浏览开发者指南和 API 参考，试用代码示例和在线演示，并下载 DHTMLX Diagram 的免费 30 天评估版本。
---
...
该事件的回调函数使用一个包含以下参数的对象进行调用：
  ...
  - `coords` - 一个包含移动后形状位置的 `x` 和 `y` 坐标的对象，其中：
    - `x` - 形状的水平位置，从左向右移动
    - `y` - 形状的垂直位置，从上向下移动
  - `event` - 事件对象
